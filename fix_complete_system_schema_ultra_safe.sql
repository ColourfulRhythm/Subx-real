-- =====================================================
-- COMPLETE SYSTEM SCHEMA FIX (ULTRA SAFE)
-- =====================================================
...
-- STEP 2: MIGRATE EXISTING DATA (ULTRA SAFE - ONLY KNOWN COLUMNS)
-- =====================================================

-- Migrate existing users from user_profiles (ULTRA SAFE)
-- Only use columns we know exist: id, email
INSERT INTO users_new (id, name, email, phone, referral_code, created_at)
SELECT 
  id,
  email as name, -- Use email as name since we know it exists
  email,
  '' as phone, -- Default empty string
  '' as referral_code, -- Default empty string
  NOW() as created_at -- Default current time
FROM user_profiles
ON CONFLICT (id) DO NOTHING;

-- Create basic plots (ULTRA SAFE - no migration, just create new)
...
-- Migrate existing plot ownership to units_purchased_new (ULTRA SAFE)
-- Only use columns we know exist: id, user_id, plot_id
...
-- Migrate existing investments to purchases_new (ULTRA SAFE)
-- Only use columns we know exist: id, user_id, amount
