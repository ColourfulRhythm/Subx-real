...
-- STEP 2: MIGRATE EXISTING DATA (PRESERVE 100%)
-- =====================================================

-- Migrate existing users from user_profiles
...
-- Migrate existing projects to plots_new
INSERT INTO plots_new (id, name, total_size, created_at)
SELECT 
  id,
  name,
  COALESCE(total_size, 500) as total_size,
  COALESCE(created_at, NOW())
FROM projects
ON CONFLICT (id) DO NOTHING;

-- Migrate existing plot ownership to units_purchased_new
...
-- Migrate existing investments to purchases_new
...
-- Set up referral relationships based on existing data
-- (This will be populated based on your existing referral logic)
